...
页面右上角的“修改课程”可以直接新增、编辑或删除课程，并为课程选择易于区分的色标。修改会立即反映到“今天”和“本周”，并保存在当前设备的浏览器中；不同设备之间不会自动共享本地修改。
...
修改后运行 `npm test`。数据校验会检查重复 ID、无效星期、无效节次、缺失地点和超出学期范围的周次。部署更新后，设备在线打开页面即可获取最新课表；离线时继续使用最后一次成功加载的版本。
